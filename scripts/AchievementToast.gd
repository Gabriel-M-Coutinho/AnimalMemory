...
func _get_info(id: String) -> Dictionary:
	...
	if id == "win_1":
		return {"title": "Primeira vitória!", "desc": "Você ganhou uma partida."}
	if id == "win_5":
		return {"title": "Campeãozinho!", "desc": "Você ganhou 5 partidas."}
	if id == "win_10":
		return {"title": "Mestre da Memória!", "desc": "Você ganhou 10 partidas."}
	if id == "win_no_hint_1":
		return {"title": "Sem ajuda!", "desc": "Ganhou uma partida sem usar dica."}
	if id == "win_no_hint_5":
		return {"title": "Pro jogador!", "desc": "Ganhou 5 partidas sem usar dica."}
	...
	if id.begins_with("card_"):
		...
		if parts.size() >= 3:
			...
			return {"title": "Colecionando!", "desc": "%s encontrado %sx" % [name, threshold]}

	return {"title": "Conquista desbloqueada!", "desc": ""}
